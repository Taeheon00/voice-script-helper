...
def load_asr_model():
    ...
    print(f"[*] ASR 모델 로딩 중... ({DEFAULT_MODEL_PATH})")
    try:
...
def apply_uvr5_vocal_extraction(input_audio_path):
    ...
    if not HAS_UVR5:
        raise RuntimeError("audio-separator 패키지가 없습니다.")
...
def execute_analysis_flow(model, file_path, active_speakers, is_single):
    print(f"\n[*] 분석 실행 중: {Path(file_path).name}")
    try:
        if model is None:
            print("[오류] 모델이 없습니다.")
            return
        ...
        with open(post_file_path, "w", encoding="utf-8") as post_file_obj:
            ...
            for idx, (start, end, orig_speaker) in enumerate(raw_speaker_turns):
                ...
                chunk_text = ""
                if chunk_audio.size > TARGET_SAMPLE_RATE * 0.2:
                    try:
                ...
                save_wav_chunk(chunk_audio, TARGET_SAMPLE_RATE, specific_segment_dir / f"{base_seg_name}.wav")
                (specific_segment_dir / f"{base_seg_name}.txt").write_text(chunk_txt, encoding="utf-8")

                log_line = f"[{mapped_speaker}] ({format_time(start)} ~ {format_time(end)}): {chunk_txt}"
        ...
        print(f"\n[+ 성공] 분석 완료! 세그먼트 경로: {specific_segment_dir}/")

    except Exception as e:
        print(f"[오류] 분석 실행 중 예외: {e}")
        traceback.print_exc()

def select_and_process_audio_file(model=None):
    """메인 메뉴 2번: '오디오' 저장소 선택 화면 (동적 번호 부여 및 자동녹화 포함)"""
    ensure_directories()
    ...
    while True:
        ...
        if item_type == 'auto':
            while True:
                ...
                print(f"[*] 선택된 폴더: {target_folder.name}")
                ...
                if not sub_wavs:
                    ...
                    continue
                target_file = sub_wavs[0]

                current_model = model
                if current_model is None:
                    current_model = load_asr_model()
                    if current_model is None:
                ...
                try:
                    ...
                    if active_speakers is not None:
                        execute_analysis_flow(current_model, str(target_file), active_speakers, is_single)
                    else:
            ...
            target_file = item_data
            print(f"[*] 선택된 파일: {target_file.name}")
